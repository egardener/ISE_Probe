...
class tcCallback : public BLECharacteristicCallbacks, ISE_Probe {
  void onWrite(BLECharacteristic *pCharacteristic) {
    std::string s = pCharacteristic->getValue();
    float f       = ::atof(s.c_str());

    ISE_Probe::useTemperatureCompensation(f);
  }

  void onRead(BLECharacteristic *pCharacteristic) {
    String s = String(ISE_Probe::usingTemperatureCompensation());

    pCharacteristic->setValue(s.c_str());
  }
};

class dpCallback : public BLECharacteristicCallbacks, ISE_Probe {
  void onWrite(BLECharacteristic *pCharacteristic) {
    std::string s = pCharacteristic->getValue();
    float f       = ::atof(s.c_str());

    ISE_Probe::useDualPoint(f);
  }

  void onRead(BLECharacteristic *pCharacteristic) {
    String s = String(ISE_Probe::usingDualPoint());
